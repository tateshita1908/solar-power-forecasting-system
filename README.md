...
* `notebooks/sql_analysis.ipynb` — supplementary SQL (DuckDB) demo: re-derives data understanding and data preparation for both plants in SQL, plus additional query patterns (window functions, CTEs, grouped aggregation, time bucketing). Not part of the main pipeline — the rest of the project uses pandas.
...
* `app.py` — Streamlit dashboard: actual vs predicted power and daily energy yield (kWh) for the test window, plus a manual prediction form.
